...
│  ├── AgentGitStateAdapter      (DEFERRED - future)                                │
│  └── CustomStateAdapter        (extensibility)                                    │
│                                                                                    │
│  IEvaluator                                                                        │
│  ├── AccuracyEvaluator                                                            │
│  ├── LatencyEvaluator                                                             │
...
> **⚠️ UPDATE (2026-01-15)**: LangGraph adapter is now PRIMARY. See §20 for details.
>
> **State Adapter Priority:**
>
> 1. `LangGraphStateAdapter` - PRIMARY (production-proven, robust)
> 2. `InMemoryStateAdapter` - For unit testing
> 3. `AgentGitStateAdapter` - DEFERRED (interface marked only)
...
class IStateAdapter(ABC):
    ...
    1. LangGraphStateAdapter: PRIMARY - Production (see §20)
    2. InMemoryStateAdapter: For unit testing
    3. AgentGitStateAdapter: DEFERRED - marked only
    """
...
### AgentGit Adapter Implementation (DEFERRED - Reference Only)

> **⚠️ Status**: This implementation is DEFERRED. See §20 for LangGraph adapter (PRIMARY).
> Keeping this section as reference for interface design.

```python
# wtb/infrastructure/adapters/agentgit_adapter.py (DEFERRED)
...
| ---------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------ |
| **State Adapter Primary?**         | ✅ LangGraph                                | Production-proven, built-in time travel, thread isolation, multiple backends                                                                           | 🆕**Approved (2026-01-15)**    |
| **AgentGit Adapter Status?**       | ⏸️ DEFERRED                               | Interface marked only; LangGraph takes priority for system robustness                                                                                  | 🆕**Approved (2026-01-15)**    |
| **Replace all SQLite?**            | ❌ No, Hybrid approach                      | SQLite for embedded/dev, PostgreSQL for production (FileTracker, WTB audit), SQLAlchemy abstraction for WTB                                            | ✅ Tests confirm SQLite works        |
...
| **Batch Test Parallelism?**        | ✅ Ray ActorPool                            | Distributed execution with connection reuse, backpressure, cancellation support. ThreadPool available for dev/testing.                                 | ✅**Implemented (2026-01-14)** |
| **Ray Actor vs Task?**             | ✅ Actor                                    | Database connection reuse across executions; isolated per-actor UoW/StateAdapter                                                                       | ✅**Implemented (2026-01-14)** |
| **Ray Backpressure?**              | ✅ ray.wait() + max_pending_tasks           | Prevents OOM with large batch tests; configurable via RayConfig                                                                                        | ✅**Implemented (2026-01-14)** |
| **Variant Execution Isolation?**   | ✅ Per-actor dependencies                   | Each VariantExecutionActor has independent UoW/StateAdapter; ACID-compliant                                                                            | ✅**Implemented (2026-01-14)** |
| **ThreadPool→Ray Migration?**     | ✅ ParityChecker                            | Dry-run validation comparing ThreadPool and Ray results; statistical comparison with tolerance                                                         | ✅**Implemented (2026-01-15)** |

### Key Design Principles

| Principle                             | Description                                                                                                                  |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **LangGraph-First Persistence** | Use LangGraph checkpointers (`PostgresSaver`, `SqliteSaver`) as PRIMARY state persistence. See §20.                     |
...
| **Interface Abstraction**       | IStateAdapter, IUnitOfWork, IRepository enable swappable implementations (InMemory for tests, LangGraph for production).     |
| **Dependency Injection**        | UnitOfWorkFactory.create(mode) enables test/production switching. No hard-coded database dependencies.                       |
| **Thread-Based Isolation**      | LangGraph native `thread_id` enables isolated checkpoint chains for parallel batch test execution.                         |
...
| ------------------------------- | ------------- | ---------------------------------- | ------------------------------------ |
| **LangGraphStateAdapter** | P0            | LangGraph checkpointers            | 🆕**PRIMARY** - See §20       |
...
| **AgentGitStateAdapter**  | ⏸️ DEFERRED | IStateAdapter                      | Marked only - LangGraph priority     |
...
## 20. LangGraph State Adapter Design (2026-01-15)

**Decision**: Use LangGraph checkpointers as PRIMARY state persistence for system robustness.

### 20.1 Why LangGraph over AgentGit?

| Criteria                        | LangGraph                                            | AgentGit                  | Winner    |
| ------------------------------- | ---------------------------------------------------- | ------------------------- | --------- |
| **Battle-tested**         | LangSmith production deployment                      | Custom implementation     | LangGraph |
| **Thread isolation**      | Native `thread_id` for parallel execution          | Manual session management | LangGraph |
| **Time travel**           | Built-in `get_state_history()`, `update_state()` | Custom rollback logic     | LangGraph |
| **Checkpointer backends** | InMemory, SQLite, PostgreSQL                         | SQLite only               | LangGraph |
| **Serialization**         | `JsonPlusSerializer` + encryption support          | Custom JSON               | LangGraph |
| **Fault tolerance**       | Pending writes preserved on node failure             | Manual recovery           | LangGraph |
| **Maintenance**           | LangChain community maintains                        | Internal team             | LangGraph |

### 20.2 LangGraph Persistence Concepts
...
│                                                                                    │
│  1. Thread (thread_id)                                                             │
│     • Unique identifier for checkpoint isolation                                   │
│     • Contains accumulated state of a sequence of runs                             │
│     • REQUIRED for checkpointer operations                                         │
│     • WTB Use Case: One thread per batch test variant execution                   │
│                                                                                    │
│  2. Checkpoint (StateSnapshot)                                                      │
│     • Saved at each super-step (after each node execution)                         │
│     • Properties:                                                                   │
│       - config: {thread_id, checkpoint_id, checkpoint_ns}                          │
│       - metadata: {source, writes, step}                                           │
│       - values: State channel values at this point                                 │
│       - next: Tuple of next node names to execute                                  │
│       - tasks: PregelTask objects with execution info                              │
│                                                                                    │
│  3. Checkpointer (BaseCheckpointSaver)                                              │
│     • Pluggable persistence backend                                                 │
│     • Interface: put, put_writes, get_tuple, list                                  │
│     • Async versions: aput, aput_writes, aget_tuple, alist                         │
│                                                                                    │
└────────────────────────────────────────────────────────────────────────────────────┘
```

### 20.3 Checkpointer Selection Matrix

| Environment           | Checkpointer      | Package                            | Use Case                |
| --------------------- | ----------------- | ---------------------------------- | ----------------------- |
| **Unit Tests**  | `InMemorySaver` | `langgraph-checkpoint` (bundled) | Fast, no I/O            |
| **Development** | `SqliteSaver`   | `langgraph-checkpoint-sqlite`    | Local persistence       |
| **Production**  | `PostgresSaver` | `langgraph-checkpoint-postgres`  | Concurrent writes, ACID |

### 20.4 LangGraph Key APIs
...
# 4. Execute with Thread Isolation
config = {"configurable": {"thread_id": "execution-001"}}
result = graph.invoke({"data": {}, "current_node": "", "execution_path": []}, config)
...
# 6. Time Travel - Get History
history = list(graph.get_state_history(config))
for snapshot in history:
    print(f"Step {snapshot.metadata['step']}: {snapshot.values}")

# 7. Replay from Specific Checkpoint
checkpoint_config = {
    "configurable": {
        "thread_id": "execution-001",
        "checkpoint_id": "abc123"  # From history
    }
}
old_state = graph.get_state(checkpoint_config)

# 8. Update State Mid-Execution
graph.update_state(config, {"data": {"modified": True}}, as_node="preprocess")
```

### 20.5 LangGraphStateAdapter Implementation
...
class LangGraphConfig:
    ...
    checkpointer_type: str = "postgres"  # "memory", "sqlite", "postgres"
...
class LangGraphStateAdapter(IStateAdapter):
    ...
    - Thread-based isolation for parallel batch tests
    - Built-in time travel via get_state_history()
    ...
    def _create_checkpointer(self) -> BaseCheckpointSaver:
        """Factory method for checkpointer based on config."""
    ...
    def set_workflow_graph(self, graph: CompiledStateGraph) -> None:
        """Set the compiled workflow graph."""
        self._graph = graph
  
    def start_session(self, execution_id: str) -> str:
        """Start a new session (thread) for execution."""
    ...
    def get_config(self) -> dict:
        ...
        return {"configurable": {"thread_id": self._current_thread_id}}
  
    def get_current_state(self) -> dict:
        """Get current state snapshot."""
        snapshot = self._graph.get_state(self.get_config())
        return snapshot.values if snapshot else {}
  
    def get_checkpoint_history(self) -> list:
        """Get full checkpoint history for time travel."""
        return list(self._graph.get_state_history(self.get_config()))
    ...
    def update_state(self, values: dict, as_node: Optional[str] = None) -> None:
        """Update state mid-execution (human-in-the-loop)."""
        self._graph.update_state(self.get_config(), values, as_node=as_node)
  
    def execute(self, initial_state: dict) -> dict:
        """Execute workflow with automatic checkpointing."""
        return self._graph.invoke(initial_state, self.get_config())
```

### 20.6 Integration with WTB Components
...
│       │                                                                            │
│       ├── LangGraphStateAdapter (PRIMARY) ──────► LangGraph Checkpointer          │
│       │       │                                        │                           │
...
│       │                                                                            │
│       └── AgentGitStateAdapter (DEFERRED - marked only)                           │
│                                                                                    │
│                                                                                    │
│  RayBatchTestRunner                                                                │
│       │                                                                            │
...
│       │ each actor has                                                             │
│       ▼                                                                            │
│  LangGraphStateAdapter with unique thread_id = f"batch-{batch_id}-{variant_id}"   │
│                                                                                    │
│       → Perfect isolation via LangGraph's native thread_id support                │
│                                                                                    │
└────────────────────────────────────────────────────────────────────────────────────┘
```

### 20.7 Benefits for WTB

| Feature                           | LangGraph Benefit                                   | WTB Use Case                 |
| --------------------------------- | --------------------------------------------------- | ---------------------------- |
| **Thread Isolation**        | Each `thread_id` has independent checkpoint chain | Batch test parallelism       |
| **Automatic Checkpointing** | Every super-step saved automatically                | No manual checkpoint calls   |
| **Time Travel**             | `get_state_history()` returns full history        | Rollback, debugging          |
| **State Update**            | `update_state()` modifies mid-execution           | Human-in-the-loop, debugging |
| **Pending Writes**          | Successful node writes preserved on failure         | Fault tolerance              |
| **Production DB**           | PostgresSaver with proper locking                   | Concurrent writes            |

### 20.8 Migration from AgentGit

| AgentGit Concept              | LangGraph Equivalent                     |
| ----------------------------- | ---------------------------------------- |
| `InternalSession`           | `thread_id`                            |
| `Checkpoint`                | `StateSnapshot`                        |
| `checkpoint_id`             | `checkpoint_id` in config              |
| `restore_from_checkpoint()` | `get_state(config_with_checkpoint_id)` |
| Manual checkpoint creation    | Automatic at each super-step             |
| `workflow_variables`        | `state.values`                         |

### 20.9 State Adapter Priority (Updated)

| Priority    | Adapter                   | Status                 | Rationale                             |
| ----------- | ------------------------- | ---------------------- | ------------------------------------- |
| **1** | `LangGraphStateAdapter` | 🆕**P0 TODO**    | Production-proven, robust             |
| **2** | `InMemoryStateAdapter`  | ✅ Done                | Unit testing, no I/O                  |
| **3** | `AgentGitStateAdapter`  | ⏸️**DEFERRED** | Interface marked; implement if needed |

### 20.10 TODO: Implementation Tasks

| Task                                  | Priority | Description                                  |
| ------------------------------------- | -------- | -------------------------------------------- |
| Create `LangGraphStateAdapter`      | P0       | Implement IStateAdapter using LangGraph      |
| Add `LangGraphConfig`               | P0       | Config dataclass with factory methods        |
| Update `ExecutionControllerFactory` | P0       | Support LangGraph adapter creation           |
| Update `VariantExecutionActor`      | P1       | Use LangGraph adapter with thread_id         |
| Add tests                             | P0       | Unit tests for adapter                       |
| Update `pyproject.toml`             | P0       | Add langgraph-checkpoint-postgres dependency |

---

## §13 Critical Decisions (2026-01-15)

### 13.1 Ray + FileTracker Integration - ✓ IMPLEMENTED

**Decision:** Integrate FileTracker with Ray batch test execution for distributed file version control.

**Problem:**

- Ray workers execute workflow variants but don't track file changes
- Rollback restores workflow state but not file system state
- No audit trail for files generated during batch tests

**Solution:** Add `IFileTrackingService` interface with multiple implementations:

| Implementation              | Use Case                         |
| --------------------------- | -------------------------------- |
| `MockFileTrackingService` | Unit testing without I/O         |
| `FileTrackerService`      | Production with PostgreSQL       |
| `RayFileTrackerService`   | Ray actors (serializable config) |

**Architecture:**

```
┌────────────────────────────────────────────────────────────────────────────────────┐
│                    RAY + FILETRACKER INTEGRATION                                    │
├────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                    │
│  RayBatchTestRunner                                                                │
│       │                                                                            │
│       │ passes filetracker_config.to_dict()                                        │
│       ▼                                                                            │
│  VariantExecutionActor                                                             │
│       │                                                                            │
│       │ lazy init                                                                  │
...
│       │ wraps                                                                      │
│       ▼                                                                            │
│  FileTrackerService ──────► file_processing/FileTracker (PostgreSQL)              │
│       │                                                                            │
│       ├── BlobRepository (content-addressed storage)                              │
│       ├── CommitRepository                                                         │
│       └── checkpoint_files table (checkpoint ↔ commit link)                       │
│                                                                                    │
└────────────────────────────────────────────────────────────────────────────────────┘
```
...
| **LSP** | Mock, FileTracker, Ray services are interchangeable  |
| **ISP** | Focused interface with single responsibility         |
| **DIP** | High-level components depend on abstraction          |

**Files:**

- `wtb/domain/interfaces/file_tracking.py` - Interface
- `wtb/config.py` - FileTrackingConfig
- `wtb/infrastructure/file_tracking/` - Implementations
- `tests/test_wtb/test_file_tracking.py` - 31 unit tests
- `tests/test_wtb/test_ray_filetracker_integration.py` - 20 integration tests

### 13.2 Single-Run File Tracking Integration - ✓ IMPLEMENTED (2026-01-17)

**Decision:** Add lightweight file tracking to single `bench.run()` execution path.

**Problem:**

- Ray batch execution had file tracking via `RayFileTrackerService` ✅
- Single-run `ExecutionController.run()` did NOT track files ❌
- Rollback only restored LangGraph state, not file system state
- Demo incorrectly claimed "File system restored"

**Solution:** 

1. Create `SqliteFileTrackingService` - lightweight SQLite-based tracking (no PostgreSQL)
2. Wire `IFileTrackingService` into `ExecutionController` as optional dependency
3. Update factories with `enable_file_tracking` parameter

| Implementation                | Use Case                                |
| ----------------------------- | --------------------------------------- |
| `MockFileTrackingService`   | Unit testing (no real file ops)         |
| `SqliteFileTrackingService` | Single-run execution (SQLite, no PG)    |
| `FileTrackerService`        | Production with PostgreSQL              |
| `RayFileTrackerService`     | Ray actors (serializable config)        |
...
│  ExecutionController                                                              │
│       │                                                                           │
│       ├── IStateAdapter (LangGraph)  ──► state rollback                          │
│       │                                                                           │
│       └── IFileTrackingService (optional)                                        │
│               │                                                                   │
│               ▼                                                                   │
│       SqliteFileTrackingService                                                   │
│               │                                                                   │
│               ├── SQLite DB (filetrack.db)                                       │
│               └── Blob storage (.filetrack/blobs/)                               │
│                                                                                   │
│  On rollback:                                                                     │
│       1. StateAdapter.rollback() → restore LangGraph state                       │
│       2. FileTrackingService.restore_from_checkpoint() → restore files           │
│                                                                                   │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
- `wtb/infrastructure/file_tracking/sqlite_service.py` - SqliteFileTrackingService
- `wtb/application/services/execution_controller.py` - Added file_tracking_service param
- `wtb/application/factories.py` - Added enable_file_tracking option

### 13.3 LangGraph Checkpointer Priority - ✓ APPROVED

**Decision:** Use LangGraph checkpointers as PRIMARY state persistence.

See §20 for detailed design.

### 13.4 Architecture Consolidation - Avoid Parallel Class Hierarchies - ✓ IMPLEMENTED (2026-01-15)

**Decision:** Maintain ONE ExecutionController, ONE NodeReplacer, TWO BatchRunners (local/distributed).

**Problem Identified:**

The initial LangGraph integration created parallel class hierarchies:
- `ExecutionController` + `LangGraphExecutionController` (duplicate orchestration)
- `NodeReplacer` + `LangGraphNodeReplacer` (duplicate variant management)
- `ThreadPoolBatchTestRunner` + `RayBatchTestRunner` + `LangGraphBatchTestRunner` (three runners)

This is an **anti-pattern** that leads to:
1. Code duplication (DRY violation)
2. Maintenance burden (changes replicated everywhere)
3. OCP violation (new backend = create 3+ new classes)
4. Shotgun surgery (related changes scattered across files)

**Correct Architecture:**

```
SINGLE Controller + Strategy/Adapter Pattern:
═══════════════════════════════════════════════════════════════════════════════

                           ┌─────────────────────────────────────┐
                           │     ExecutionController (ONE)       │
                           │     - Orchestration logic ONLY      │
                           │     - Depends on IStateAdapter      │
                           └──────────────┬──────────────────────┘
                                          │ uses
                                          ▼
                    ┌─────────────────────────────────────────────────┐
                    │              IStateAdapter (extended)            │
                    │  Core methods (required):                        │
                    │  - save_checkpoint(), load_checkpoint()          │
                    │  - rollback(), get_checkpoints()                 │
                    │                                                  │
                    │  Extended methods (optional - for LangGraph):    │
                    │  - get_checkpoint_history() → time-travel        │
                    │  - update_state() → human-in-the-loop           │
                    │  - supports_streaming() → event streaming        │
                    │  - supports_time_travel() → capability check     │
                    └────────────────────┬────────────────────────────┘
                                         │ implemented by
             ┌───────────────────────────┼───────────────────────────┐
             ▼                           ▼                           ▼
┌────────────────────────┐  ┌────────────────────────┐  ┌────────────────────────┐
│  InMemoryStateAdapter  │  │  AgentGitStateAdapter  │  │ LangGraphStateAdapter  │
│  (testing)             │  │  (git-based)           │  │ (checkpointer-based)   │
│  supports_time_travel: │  │  supports_time_travel: │  │ supports_time_travel:  │
│  FALSE                 │  │  FALSE                 │  │ TRUE ✓                 │
└────────────────────────┘  └────────────────────────┘  └────────────────────────┘

BatchRunners (Strategy Pattern - local vs distributed):
═══════════════════════════════════════════════════════════════════════════════
                     ...
                     ▼                                         ▼
        ┌────────────────────────┐             ┌────────────────────────┐
        │ ThreadPoolBatchRunner  │             │   RayBatchRunner       │
        │ (local parallelism)    │             │   (distributed)        │
        └────────────────────────┘             └────────────────────────┘
                     │                                         │
                     └─────────────────────────────────────────┘
                                       │
                                       │ BOTH inject IStateAdapter
                                       │ (can be LangGraphStateAdapter!)
                                       ▼
                    ┌─────────────────────────────────────┐
                    │  ExecutionController + IStateAdapter │
                    └─────────────────────────────────────┘
```
...
| Principle | Implementation |
|-----------|----------------|
| **DIP** | Controller depends on `IStateAdapter` abstraction, not concrete adapters |
| **OCP** | New backend = new adapter implementation ONLY, no new controller |
| **SRP** | Controller: orchestration. Adapter: persistence. Runner: parallelism strategy |
| **ISP** | Extended IStateAdapter methods are optional (have default implementations) |
| **Strategy Pattern** | BatchRunners choose execution strategy (local vs distributed) |
| **Adapter Pattern** | IStateAdapter adapts different backends to unified interface |

**Deprecation:**

| Component | Status | Reason |
|-----------|--------|--------|
| `LangGraphExecutionController` | **DEPRECATED** | Use `ExecutionController` + `LangGraphStateAdapter` |
| `LangGraphBatchTestRunner` | **DEPRECATED** | Use `ThreadPoolBatchRunner` or `RayBatchRunner` + inject `LangGraphStateAdapter` |
| `LangGraphNodeReplacer` | **KEPT** | Different domain - works with LangGraph `StateGraph` directly for native LangGraph development |

**Migration Path:**

```python
# OLD (deprecated):
from wtb.application.services import LangGraphExecutionController
controller = LangGraphExecutionController(state_adapter=lg_adapter, ...)
...
# Time-travel now available via extended IStateAdapter methods:
...
- `wtb/domain/interfaces/state_adapter.py` - Added extended methods with defaults
- `wtb/infrastructure/adapters/langgraph_state_adapter.py` - Implements extended methods
- `wtb/application/services/execution_controller.py` - Added capability-aware methods

### 13.4 Two BatchRunners is Correct Design - ✓ CONFIRMED (2026-01-15)

**Question:** Why are there two batch runners (`ThreadPoolBatchTestRunner` and `RayBatchTestRunner`)?

**Answer:** This is the **correct** application of the **Strategy Pattern**.

**Rationale:**

| Dimension | ThreadPoolBatchRunner | RayBatchRunner |
|-----------|----------------------|----------------|
| **Execution Model** | Local multi-threading | Distributed actors |
| **Scalability** | Single machine (4-16 workers) | Cluster (100+ workers) |
| **Isolation** | Thread isolation | Process/container isolation |
| **Deployment** | No setup required | Requires Ray cluster |
| **Use Case** | Development, CI/CD | Production batch testing |
| **Resource Sharing** | Shared memory | Object store |

**Why NOT a third LangGraphBatchRunner?**

LangGraph is a **state persistence backend**, not an **execution strategy**. The batch runner's job is to parallelize execution (local vs distributed). The state adapter's job is to persist state (InMemory vs LangGraph vs AgentGit).

These are orthogonal concerns:

```
Execution Strategy (Runner)    ×    State Backend (Adapter)
─────────────────────────────────────────────────────────────
ThreadPoolBatchRunner          ×    InMemoryStateAdapter     ✓
ThreadPoolBatchRunner          ×    LangGraphStateAdapter    ✓ (CORRECT!)
RayBatchRunner                 ×    InMemoryStateAdapter     ✓
RayBatchRunner                 ×    LangGraphStateAdapter    ✓ (CORRECT!)
```

Creating a `LangGraphBatchRunner` would conflate these concerns and lead to the combinatorial explosion problem:
- N backends × M strategies = N×M implementations (bad!)
- N backends + M strategies = N+M implementations (good!)

**SOLID Compliance:**

| Principle | How BatchRunners Comply |
|-----------|------------------------|
| **SRP** | Each runner has ONE job: execution strategy |
| **OCP** | New strategy (e.g., Kubernetes) = new runner, doesn't change existing |
| **LSP** | Both implement `IBatchTestRunner`, fully substitutable |
| **ISP** | `IBatchTestRunner` is focused (run, cancel, get_status) |
| **DIP** | Runners depend on `IStateAdapter`, not concrete adapters |

### 13.5 Checkpointer Wiring Architecture - ✓ IMPLEMENTED (2026-01-17)

**Decision:** StateAdapter (Application layer) is responsible for wiring checkpointers to graphs, NOT the SDK.

**Problem Identified:**

The original implementation had the SDK extracting checkpointer from StateAdapter and passing it to `build_graph()`:

```python
# WRONG (SDK controlling infrastructure):
checkpointer = self._state_adapter.get_checkpointer()
graph = proj.build_graph(checkpointer=checkpointer)  # SDK wiring checkpointer!
```

This violated:
- **Single Responsibility Principle (SRP)**: SDK was handling infrastructure concerns
- **Dependency Inversion Principle (DIP)**: SDK was controlling implementation details
- **ACID Compliance Risk**: If factory returned pre-compiled graph, checkpoints weren't persisted

**Solution:** Move checkpointer wiring to `LangGraphStateAdapter.set_workflow_graph()`:

```python
# CORRECT (Application layer controls checkpointer):
graph = proj.build_graph()  # SDK doesn't pass checkpointer
self._state_adapter.set_workflow_graph(graph, force_recompile=True)  # Adapter wires checkpointer
```
...
```
┌────────────────────────────────────────────────────────────────────────────────────┐
│                    CHECKPOINTER WIRING ARCHITECTURE                                 │
├────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                    │
│  SDK Layer (test_bench.py)                                                         │
│  ├── run() builds graph WITHOUT checkpointer                                       │
│  ├── Does NOT extract checkpointer from adapter                                    │
│  └── Passes graph to ExecutionController                                           │
│                                                                                    │
│  Application Layer (execution_controller.py)                                       │
│  ├── _run_with_langgraph() calls set_workflow_graph(graph, force_recompile=True)  │
│  └── StateAdapter handles checkpointer wiring                                      │
│                                                                                    │
│  Infrastructure Layer (langgraph_state_adapter.py)                                 │
│  ├── set_workflow_graph() ALWAYS recompiles with own checkpointer                  │
│  ├── get_checkpointer() exposes checkpointer for inspection                        │
│  └── Ensures ACID compliance for checkpoint persistence                            │
│                                                                                    │
└────────────────────────────────────────────────────────────────────────────────────┘
```
...
| File | Change |
|------|--------|
| `wtb/sdk/test_bench.py` | Removed checkpointer extraction; `build_graph()` called without checkpointer |
| `wtb/sdk/workflow_project.py` | `build_graph()` no longer accepts checkpointer parameter |
| `wtb/infrastructure/adapters/langgraph_state_adapter.py` | `set_workflow_graph(force_recompile=True)` recompiles with adapter's checkpointer |
| `wtb/application/services/execution_controller.py` | Calls `set_workflow_graph(graph, force_recompile=True)` |

**SOLID Compliance:**

| Principle | Implementation |
|-----------|----------------|
| **SRP** | SDK: user interface. Adapter: checkpointer wiring. Separate concerns. |
| **OCP** | New checkpointer types = new adapter config, no SDK changes |
| **LSP** | All adapters implement IStateAdapter, LangGraph-specific methods optional |
| **ISP** | LangGraph methods (`set_workflow_graph`, `get_checkpointer`) only on LangGraphStateAdapter |
| **DIP** | ExecutionController depends on IStateAdapter abstraction |

**Adapter Design:**

| Adapter | LangGraph Methods | Execution Path |
|---------|------------------|----------------|
| `LangGraphStateAdapter` | `set_workflow_graph()`, `get_checkpointer()`, `execute()` | `_run_with_langgraph()` |
| `InMemoryStateAdapter` | None | `_run_with_node_executor()` |
| `AgentGitStateAdapter` | None | `_run_with_node_executor()` |

**Test Coverage:**

- `tests/test_langgraph/unit/test_checkpointer_wiring.py` - 7 unit tests
- `tests/test_langgraph/integration/test_checkpointer_persistence.py` - 5 integration tests

### 13.6 External API Architecture - ✓ APPROVED (2026-01-15)

**Decision:** Use REST + gRPC + WebSocket for external system control.

**Problem:**

- External users/services need to control WTB (execute workflows, pause, rollback)
- Different clients have different needs (Web UI, CLI, other services, Ray workers)
- Real-time updates required for execution progress and audit events
- Need high-throughput communication for Ray worker coordination

**Solution:** Multi-protocol API architecture:

| Protocol | Port | Use Case |
|----------|------|----------|
| **REST** (FastAPI) | 8000 | Web UI, external clients, CRUD operations, debugging |
| **gRPC** | 50051 | Internal services, streaming events, Ray workers, high-throughput |
| **WebSocket** | 8000/ws | Real-time UI updates, live audit trail, execution progress |
...
│  External Clients                                                                │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐    │
│  │   Web UI    │  │   CLI Tool  │  │  Ray Workers│  │ External Services   │    │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └──────────┬──────────┘    │
│         │ WS+REST        │ REST           │ gRPC              │ REST/gRPC      │
│         └────────────────┴────────────────┴───────────────────┘                 │
│                                   │                                              │
│                                   ▼                                              │
│         ┌─────────────────────────────────────────────────────────────────┐     │
│         │                      API Gateway Layer                           │     │
│         │  REST (FastAPI)    │   gRPC (Async)   │   WebSocket (FastAPI)   │     │
│         └─────────────────────────────────────────────────────────────────┘     │
│                                   │                                              │
│                                   ▼                                              │
│         ┌─────────────────────────────────────────────────────────────────┐     │
│         │                   Application Services                           │     │
│         │  ExecutionService │ BatchTestService │ AuditService │ MetricsSvc│     │
│         └─────────────────────────────────────────────────────────────────┘     │
│                                   │                                              │
│                                   ▼                                              │
│         ┌─────────────────────────────────────────────────────────────────┐     │
│         │                      Domain Layer                                │     │
│         │  ExecutionController │ RayBatchTestRunner │ WTBEventBus │ Audit │     │
│         └─────────────────────────────────────────────────────────────────┘     │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
├── /executions            # Execution control (start, pause, resume, stop, rollback)
├── /batch-tests           # Batch test management
├── /audit                 # Audit event queries and timeline
...
| Principle | Implementation |
|-----------|----------------|
| **SRP** | Each protocol handler has single responsibility |
| **OCP** | New protocols via IAPIProtocol interface |
| **LSP** | REST/gRPC handlers interchangeable via common service interfaces |
| **ISP** | Separate interfaces: IExecutionControl, IAuditQuery, IMetricsExport |
| **DIP** | API layer depends on service abstractions, not implementations |

**Files:**

- `docs/external_api/EXTERNAL_API_AND_OBSERVABILITY_DESIGN.md` - Complete design document
- `wtb/api/rest/` - FastAPI routes ✅ IMPLEMENTED (2026-01-15)
  - `app.py` - FastAPI application factory
  - `models.py` - Pydantic schemas (30+ models)
  - `dependencies.py` - Dependency injection
  - `routes/` - workflows, executions, audit, batch_tests, health
- `wtb/api/grpc/` - gRPC service definition ✅ IMPLEMENTED (2026-01-15)
  - `protos/wtb_service.proto` - Service definition
- `wtb/api/websocket/` - WebSocket handlers ✅ IMPLEMENTED (2026-01-15)
  - `handlers.py` - ConnectionManager + event streaming
- `wtb/sdk/` - Workflow Submission SDK ✅ IMPLEMENTED (2026-01-15)
  - `workflow_project.py` - WorkflowProject configuration
  - `test_bench.py` - WTBTestBench main interface
- `tests/test_api/` - Unit and integration tests ✅ IMPLEMENTED (2026-01-15)

### 13.7 Observability Architecture - ✓ APPROVED (2026-01-15)

**Decision:** Use OpenTelemetry → Prometheus → Grafana for observability.

**Problem:**

- Need visibility into execution performance, errors, and audit events
- Different consumers: dashboards, alerts, debugging
- Must be vendor-neutral for cloud portability
- Existing `MetricsEventListener` uses Prometheus directly

**Solution:** OpenTelemetry as the abstraction layer:
...
| Metric | Type | Purpose |
|--------|------|---------|
| `wtb_executions_total` | Counter | Total executions by status |
| `wtb_execution_duration_seconds` | Histogram | Execution latency distribution |
| `wtb_node_duration_seconds` | Histogram | Node execution times |
...
| Method | Use Case |
|--------|----------|
| **REST API** `/api/v1/audit/events` | Query audit events with filtering |
| **gRPC Stream** `StreamAuditEvents()` | Real-time subscription for services |
| **WebSocket** `subscribe("audit:*")` | Real-time updates for Web UI |
| **Prometheus Metrics** | Aggregated counts and rates |
...
|-----------|----------|-------------|
| Create Execution | Single DB txn | Strong |
| Create Checkpoint | Single DB txn + Outbox | Strong |
| Rollback | Multi-step Saga | Eventual |
| Audit Recording | Batch flush | Eventually consistent |

**Files:**

- `docs/external_api/EXTERNAL_API_AND_OBSERVABILITY_DESIGN.md` - Complete design
- `wtb/infrastructure/observability/metrics.py` - OpenTelemetry metrics (TODO)
- `wtb/infrastructure/observability/tracing.py` - Distributed tracing (TODO)
- `wtb/infrastructure/events/metrics_event_listener.py` - Existing Prometheus integration ✅

### 13.8 FileTracker Verification in OutboxProcessor - ✓ IMPLEMENTED (2026-01-15)

**Decision:** Enhance OutboxProcessor with comprehensive FileTracker verification handlers.

**Problem:**

- Outbox events need verification across WTB, AgentGit, and FileTracker
- Batch tests create multiple file commits that need batch verification
- Rollback operations need coordinated file restoration
- No integrity verification for blob content

**Solution:** Add new handlers and protocols to OutboxProcessor:

| Event Type | Handler | Description |
|------------|---------|-------------|
| `FILE_BATCH_VERIFY` | `_handle_file_batch_verify` | Verify multiple commits in batch tests |
| `FILE_INTEGRITY_CHECK` | `_handle_file_integrity_check` | Deep hash verification |
| `FILE_RESTORE_VERIFY` | `_handle_file_restore_verify` | Verify restored files |
| `ROLLBACK_FILE_RESTORE` | `_handle_rollback_file_restore` | Coordinate file restoration |
| `ROLLBACK_VERIFY` | `_handle_rollback_verify` | Full rollback verification |

**Architecture:**

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  OutboxProcessor - FileTracker Integration                                       │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                       Protocol Abstractions (DIP)                         │   │
│  │  ICheckpointRepository │ ICommitRepository │ IBlobRepository │ IFileTracking │
│  └───────────────┬────────────────┬─────────────────┬───────────────┬───────┘   │
│                  │                │                 │               │            │
│                  ▼                ▼                 ▼               ▼            │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                           Handler Methods                                │   │
│  │                                                                          │   │
│  │  FILE_BATCH_VERIFY     → Batch verification for batch tests             │   │
│  │  FILE_INTEGRITY_CHECK  → Deep hash verification for blobs               │   │
│  │  FILE_RESTORE_VERIFY   → Verify restored files exist                    │   │
│  │  ROLLBACK_FILE_RESTORE → Restore files from commit                      │   │
│  │  ROLLBACK_VERIFY       → Full cross-DB rollback verification            │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                   │                                              │
│                                   ▼                                              │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
...
| Principle | Implementation |
|-----------|----------------|
| **SRP** | Each handler verifies one event type |
| **OCP** | New handlers via handler dict without modification |
| **LSP** | Mock implementations substitutable for testing |
| **ISP** | Separate protocols for different repositories |
| **DIP** | OutboxProcessor depends on protocols, not concrete classes |

**Files:**

- `wtb/domain/models/outbox.py` - Added OutboxEventType values + factory methods ✅
- `wtb/infrastructure/outbox/processor.py` - Added handlers + IFileTrackingService protocol ✅
- `tests/test_wtb/test_outbox_filetracker.py` - 27 unit tests ✅

### 13.9 Workspace Isolation for Parallel Variant Execution - ✓ IMPLEMENTED (2026-01-16)

**Decision:** Implement workspace isolation using `WorkspaceManager` to create isolated directory trees for each variant execution in Ray batch tests.

**Problem:**

- Parallel variant execution via Ray can cause file system race conditions
- Multiple actors writing to same files causes data corruption
- Rollback restores state but not file system
- No isolation between variant executions

**Solution:** Add `Workspace` domain model and `WorkspaceManager` for isolated execution:

| Component | Description |
|-----------|-------------|
| `Workspace` | Value object with derived paths (input, output, venv) |
| `WorkspaceConfig` | Configuration (base_dir, strategy, isolation flags) |
| `WorkspaceStrategy` | FULL_COPY, HARDLINK_WITH_FALLBACK, SYMLINK |
| `WorkspaceManager` | Creates, activates, cleans up workspaces |
...
│  RayBatchTestRunner                                                              │
│       │                                                                          │
│       │ create_workspace() for each variant                                      │
│       ▼                                                                          │
│  WorkspaceManager                                                                │
│       │                                                                          │
│       │ creates isolated directory tree                                          │
│       ▼                                                                          │
│  base_dir/                                                                       │
│  └── batch_{batch_id}/                                                           │
│      └── variant_{variant_id}/                                                   │
...
│               ▼                                                                  │
│  WorkspaceManager.cleanup_batch()                                                │
│       │                                                                          │
│       │ removes all workspaces for batch (in finally block)                      │
│       ▼                                                                          │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| Decision | Choice | Rationale |
|----------|--------|-----------|
| **File Sharing** | Hard links with copy fallback | Efficient on same partition, works cross-partition |
| **Orphan Detection** | Lock file + PID check | Handles crashed processes without file locks |
| **Workspace Lifecycle** | activate/deactivate pattern | Clean entry/exit with state restoration |
| **Venv Tracking** | venv_spec_hash in metadata | Detects dependency changes for cache invalidation |
| **Cleanup Strategy** | Batch cleanup in finally block | Ensures cleanup even on exceptions |
...
- `wtb/domain/models/workspace.py` - Workspace, WorkspaceConfig, OrphanWorkspace ✅
- `wtb/domain/events/workspace_events.py` - 15+ workspace lifecycle events ✅
...
- `tests/test_wtb/test_workspace_manager.py` - 38 unit tests ✅
- `tests/test_wtb/test_workspace_integration.py` - 13 integration tests ✅
- `tests/test_wtb/test_ray_workspace_integration.py` - 10 Ray integration tests ✅

### 13.10 ActorLifecycleManager - Pause/Resume Strategies - ✓ IMPLEMENTED (2026-01-16)

**Decision:** Implement `ActorLifecycleManager` with Hot/Warm/Cold pause strategies to optimize resource usage during batch test execution.

**Problem:**

- Ray actors consume resources (CPU, memory, GPU) even when idle during pauses
- Long-running batch tests may pause for hours (e.g., waiting for approval)
- No standard mechanism for pausing/resuming distributed actors
- Need to balance resume speed vs resource cost

**Solution:** Three-tier pause strategy with automatic selection:

| Strategy | Actor State | Resume Time | Resource Usage | Use Case |
|----------|-------------|-------------|----------------|----------|
| **HOT** | Running, paused flag | Instant (<10ms) | Full resources held | Short pauses (<5min) |
| **WARM** | Running, minimal state | Fast (~100ms) | Reduced memory | Medium pauses (5-30min) |
| **COLD** | Killed, recreate on resume | Slow (~1-5s) | Zero resources | Long pauses (>30min) |
...
│  │                                                                          │   │
│  │  Input:  expected_duration_seconds, actor_resources (cpu, memory, gpu)  │   │
│  │  Output: PauseStrategy (HOT | WARM | COLD)                              │   │
│  │                                                                          │   │
│  │  Rules:                                                                  │   │
│  │  • duration < 5min → HOT (fast resume)                                  │   │
│  │  • duration < 30min + no GPU → WARM (balanced)                          │   │
│  │  • duration >= 30min OR has GPU → COLD (resource savings)               │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                   │                                              │
│                                   ▼                                              │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                      Actor Operations                                    │   │
│  │                                                                          │   │
│  │  create_actor()   → Creates Ray actor with tracked resources            │   │
│  │  reset_actor()    → Resets actor state for reuse (variant isolation)    │   │
│  │  pause_actor()    → Applies pause strategy (HOT/WARM/COLD)              │   │
│  │  resume_actor()   → Resumes or recreates based on strategy              │   │
│  │  fork_actor()     → Creates new actor for fork operations               │   │
│  │  kill_actor()     → Terminates actor and releases resources             │   │
│  │  handle_rollback()→ Manages actor state during rollback                 │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                   │                                              │
│                                   ▼                                              │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                      Event Integration (WTBEventBus)                     │   │
│  │                                                                          │   │
│  │  ActorCreatedEvent     → actor_id, resources, workspace_id              │   │
│  │  ActorResetEvent       → actor_id, rollback_checkpoint_id               │   │
│  │  ActorPausedEvent      → actor_id, strategy, expected_duration          │   │
│  │  ActorResumedEvent     → actor_id, was_recreated, resume_time           │   │
│  │  ActorForkedEvent      → original_id, new_id, source_checkpoint         │   │
│  │  ActorKilledEvent      → actor_id, reason, final_state                  │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| Decision | Choice | Rationale |
|----------|--------|-----------|
| **Strategy Selection** | Auto-select based on duration + resources | Balances UX (fast resume) vs cost (resource usage) |
| **GPU Priority** | Always COLD if GPU present | GPU memory is expensive; release immediately |
| **Rollback Behavior** | Reset or recreate based on state delta | Minor changes = reset; major changes = fresh actor |
| **Fork Isolation** | Always create new actor | Guarantees clean state, no contamination |
| **Handle Registry** | In-memory dict with actor metadata | Fast lookup, cleared on manager shutdown |

**SOLID Compliance:**

| Principle | Implementation |
|-----------|----------------|
| **SRP** | ActorLifecycleManager handles lifecycle ONLY, not execution |
| **OCP** | New pause strategies via PauseStrategy enum extension |
| **LSP** | All pause strategies implement same pause/resume contract |
| **ISP** | Focused interface: create, pause, resume, kill, fork |
| **DIP** | Manager depends on WTBEventBus abstraction for events |

**Files:**

- `wtb/application/services/actor_lifecycle.py` - ActorLifecycleManager implementation ✅
- `wtb/domain/events/workspace_events.py` - Actor lifecycle events ✅
- `tests/test_wtb/test_actor_lifecycle.py` - 25 unit tests ✅

### 13.11 Venv Cache with LRU Eviction - ✓ IMPLEMENTED (2026-01-16)

**Decision:** Implement `VenvCacheManager` with LRU eviction to optimize virtual environment creation performance.

**Problem:**

- Creating virtual environments is expensive (5-30s per venv)
- Parallel batch tests may need identical venvs for multiple variants
- Windows I/O makes venv creation particularly slow
- No mechanism to reuse existing venvs across runs

**Solution:** Spec hash-based caching with LRU eviction:

| Component | Description |
|-----------|-------------|
| `VenvCacheEntry` | Cached venv metadata (path, spec_hash, size, last_access) |
| `VenvCacheManager` | Cache manager with LRU eviction policy |
| `compute_venv_spec_hash()` | Deterministic hash from requirements |
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│  Cache Key: venv_spec_hash (SHA-256 of sorted requirements + Python version)   │
│                                                                                  │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                          Cache Operations                                │   │
│  │                                                                          │   │
│  │  get_by_spec(spec_hash)    → Returns cached venv path if exists         │   │
│  │  put(entry)                → Adds venv to cache, triggers eviction      │   │
│  │  copy_to_workspace(hash, dest) → Copies cached venv to workspace        │   │
│  │  invalidate(spec_hash)     → Removes specific cache entry               │   │
│  │  evict_if_needed()         → LRU eviction when over max_entries         │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                   │                                              │
│                                   ▼                                              │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
...
│  │                                                                          │   │
│  │  Triggers:                                                               │   │
│  │  • cache_entries > max_entries (default: 100)                           │   │
│  │  • entry.age > max_age_seconds (default: 7 days)                        │   │
│  │                                                                          │   │
│  │  Eviction Order:                                                         │   │
│  │  1. Entries older than max_age (age-based)                              │   │
│  │  2. Least recently used (LRU by last_access_time)                       │   │
│  │                                                                          │   │
│  │  Action:                                                                 │   │
│  │  • Remove from cache dict                                                │   │
│  │  • Delete venv directory from disk                                       │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                   │                                              │
│                                   ▼                                              │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
...
│  │  WorkspaceManager                                                        │   │
│  │       │                                                                  │   │
│  │       │ 1. compute_venv_spec_hash(requirements)                         │   │
│  │       │ 2. cache.get_by_spec(hash)                                      │   │
│  │       │    ├── HIT: cache.copy_to_workspace(hash, workspace.venv_dir)   │   │
│  │       │    └── MISS: create_venv() → cache.put(entry)                   │   │
│  │       ▼                                                                  │   │
│  │  Workspace.venv_dir                                                      │   │
│  │                                                                          │   │
│  │  Event Bus Integration:                                                  │   │
│  │  • VenvCacheHitEvent → audit trail for cache hits                       │   │
│  │  • VenvCacheMissEvent → audit trail for cache misses                    │   │
│  │  • VenvEvictedEvent → audit trail for evictions                         │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
|----------|---------------|------------|-------------|
| First variant | 15s (create venv) | 15s (create + cache) | — |
| Second variant (same deps) | 15s (create venv) | 0.5s (cache hit + copy) | **30x faster** |
| 100 variants (10 unique specs) | 1500s | 165s | **9x faster** |

**Key Design Decisions:**

| Decision | Choice | Rationale |
|----------|--------|-----------|
| **Hash Function** | SHA-256 of sorted requirements | Deterministic, collision-resistant |
| **Copy vs Link** | shutil.copytree for Windows compatibility | Hard links fail cross-partition |
| **Eviction Policy** | LRU + age-based | Balances disk space vs hit rate |
| **Max Entries** | 100 (configurable) | ~5GB typical (50MB per venv) |
| **Max Age** | 7 days (configurable) | Prevent stale dependencies |

**SOLID Compliance:**

| Principle | Implementation |
|-----------|----------------|
| **SRP** | VenvCacheManager handles caching ONLY |
| **OCP** | Eviction policy configurable via constructor |
| **LSP** | Cache entries interchangeable regardless of source |
| **ISP** | Focused interface: get, put, copy, invalidate, stats |
| **DIP** | No external dependencies, works with any venv provider |

**Files:**

- `wtb/infrastructure/environment/venv_cache.py` - VenvCacheManager implementation ✅
- `wtb/domain/models/workspace.py` - compute_venv_spec_hash() function ✅
- `wtb/domain/events/workspace_events.py` - Venv cache events ✅
- `tests/test_wtb/test_venv_cache.py` - 18 unit tests ✅

### 13.12 CheckpointFileLink Unification - ✓ IMPLEMENTED (2026-01-16)

**Decision:** Rename interface `CheckpointFileLink` to `FileTrackingLink` to eliminate confusion with domain model.

**Problem:**

- Two classes named `CheckpointFileLink` existed in different layers:
  - `domain/interfaces/file_tracking.py` - Interface VO with primitive `str` commit_id
  - `domain/models/file_processing.py` - Domain model with `CommitId` value object
- Different services used different classes causing type confusion
- Violated DDD principle of clear bounded context boundaries

**Solution:** Rename interface VO and clarify ACL boundary:

| Layer | Class | commit_id Type | Purpose |
|-------|-------|----------------|---------|
| **Interface (ACL)** | `FileTrackingLink` | `str` (primitive) | External system integration |
| **Domain Model** | `CheckpointFileLink` | `CommitId` (VO) | Domain-internal use |
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│  External Systems (FileTracker)                                                  │
│       │                                                                          │
│       │ returns FileTrackingLink (primitive types)                               │
│       ▼                                                                          │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                    IFileTrackingService                                  │   │
│  │                                                                          │   │
│  │  link_to_checkpoint() → FileTrackingLink                                │   │
│  │                         commit_id: str  ← Primitive for serialization   │   │
│  │                                                                          │   │
│  │  Implementations:                                                        │   │
│  │  • MockFileTrackingService (testing)                                    │   │
│  │  • FileTrackerService (PostgreSQL)                                      │   │
│  │  • RayFileTrackerService (distributed)                                  │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                   │                                              │
│                                   │ ACL Boundary Conversion                      │
│                                   ▼                                              │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                    Domain Layer                                          │   │
│  │                                                                          │   │
│  │  CheckpointFileLink (domain model)                                      │   │
│  │      commit_id: CommitId  ← Value object with validation               │   │
│  │                                                                          │   │
│  │  Conversion at boundary:                                                 │   │
│  │  interface_link.commit_id → CommitId(interface_link.commit_id)         │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                   │                                              │
│                                   │ Repository Layer                             │
│                                   ▼                                              │
...
│  │                                                                          │   │
│  │  add(link: CheckpointFileLink)  ← Domain model with CommitId           │   │
│  │  get_by_checkpoint() → CheckpointFileLink                               │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
```python
# At ACL boundary (entering domain from external service):
interface_link = file_tracking_service.link_to_checkpoint(cp_id, commit_id)
...
# At ACL boundary (leaving domain to external service):
domain_link = checkpoint_link_repo.get_by_checkpoint(cp_id)
interface_link = FileTrackingLink(
    checkpoint_id=domain_link.checkpoint_id,
    commit_id=domain_link.commit_id.value,  # Extract primitive
...
| Principle | Implementation |
|-----------|----------------|
| **SRP** | Each class has single responsibility (interface VO vs domain model) |
| **OCP** | Adding new link types doesn't modify existing classes |
| **LSP** | Both types substitutable within their respective layers |
| **ISP** | Interface VO has minimal serialization-focused methods |
| **DIP** | Domain depends on abstractions (CommitId), not primitives |

**DDD Compliance:**

| Concept | Implementation |
|---------|----------------|
| **Bounded Context** | Clear separation: Interface layer vs Domain layer |
| **Anti-Corruption Layer** | FileTrackingLink translates external concepts |
| **Value Objects** | CommitId provides validation and type safety |
| **Ubiquitous Language** | "FileTrackingLink" for external, "CheckpointFileLink" for domain |

**Files Modified:**

- `wtb/domain/interfaces/file_tracking.py` - Renamed CheckpointFileLink → FileTrackingLink ✅
- `wtb/domain/interfaces/__init__.py` - Updated exports ✅
- `wtb/infrastructure/file_tracking/filetracker_service.py` - Updated return type ✅
- `wtb/infrastructure/file_tracking/mock_service.py` - Updated return type ✅
- `wtb/infrastructure/file_tracking/ray_filetracker_service.py` - Updated return type ✅

**Tests Added:**

- `tests/test_file_processing/unit/test_file_tracking_link.py` - 27 unit tests ✅
- `tests/test_file_processing/integration/test_file_tracking_integration.py` - 14 integration tests ✅

**Also Fixed:**

- Deleted duplicate `database/models/file_processing_orm.py` (orphan file)
- Removed empty `database/models/` directory