# WTB Database Design (LangGraph + FileTracker)
...
> 
> **"Use Anti-Corruption Layer - WTB owns its semantic layer via adapters."**

> **"WTB uses LangGraph checkpointers for state persistence; SQLAlchemy UoW for WTB domain data."** (Updated 2026-01-15)

> **"Single Checkpoint Type + Node Boundary Pointers: All checkpoints are atomic at node (super-step) level. Node boundaries are POINTERS to LangGraph checkpoint_ids, NOT separate checkpoints."** (Updated 2026-01-15 - tool/message level DEFERRED per [CHECKPOINT_GRANULARITY_DESIGN.md](../LangGraph/CHECKPOINT_GRANULARITY_DESIGN.md))

## State Adapter Priority (2026-01-15)

| Priority | Adapter | Storage | Status |
|----------|---------|---------|--------|
| **1** | `LangGraphStateAdapter` | LangGraph Checkpointers | 🆕 **PRIMARY** |
| **2** | `InMemoryStateAdapter` | RAM | ✅ Testing |
| **3** | `AgentGitStateAdapter` | AgentGit SQLite | ⏸️ **DEFERRED** |

> **Note**: AgentGit adapter design is kept below for reference only. LangGraph is the primary implementation.

### LangGraph Checkpointer Storage (PRIMARY)
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│  LangGraph automatically saves checkpoints at each super-step.                  │
│  Checkpointers handle serialization, storage, and retrieval.                    │
│                                                                                  │
...
│  │                │  │                │  │                │                    │
│  │  Unit tests    │  │  Development   │  │  Production    │                    │
│  │  No persistence│  │  Local file    │  │  LangSmith     │                    │
│  │                │  │                │  │  Ray clusters  │                    │
│  │  Bundled with  │  │  Install:      │  │  Install:      │                    │
...
│                                                                                  │
│  STORAGE FEATURES:                                                               │
│  ├── Thread-based isolation (thread_id as primary key)                          │
│  ├── Automatic checkpoint at each super-step                                    │
│  ├── Full history via get_state_history()                                       │
│  ├── Serialization: JsonPlusSerializer (+ pickle fallback)                      │
│  ├── Encryption: EncryptedSerializer via LANGGRAPH_AES_KEY                      │
│  └── Pending writes preserved on node failure                                   │
│                                                                                  │
│  CHECKPOINT SCHEMA (Managed by LangGraph):                                       │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  checkpoints                                                             │   │
│  │  ├── thread_id (PK)        - Execution isolation                        │   │
│  │  ├── checkpoint_id (PK)    - Unique checkpoint ID                       │   │
│  │  ├── checkpoint_ns         - Namespace (for subgraphs)                  │   │
│  │  ├── parent_checkpoint_id  - Chain reference                            │   │
│  │  ├── checkpoint_data       - Serialized state                           │   │
│  │  ├── metadata              - Step, source, writes                       │   │
│  │  └── created_at            - Timestamp                                  │   │
│  │                                                                          │   │
│  │  checkpoint_writes                                                       │   │
│  │  ├── thread_id             - FK to checkpoints                          │   │
│  │  ├── checkpoint_id         - FK to checkpoints                          │   │
│  │  ├── task_id               - Pregel task identifier                     │   │
...
### Architecture Decision: LangGraph + WTB + FileTracker

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                     Three-Database Architecture (Updated 2026-01-15)             │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│   LangGraph Checkpointer (PRIMARY)          WTB Database                        │
│   ════════════════════════════════          ═══════════════                     │
│   PostgreSQL (prod) / SQLite (dev)          SQLite/PostgreSQL: wtb.db          │
│                                                                                  │
│   ┌─────────────────────────┐               ┌─────────────────────────┐         │
│   │ checkpoints (managed    │               │ wtb_workflows           │         │
│   │   by LangGraph)         │               │ wtb_executions          │         │
│   │ ├── thread_id           │◄──────────────│ wtb_node_boundaries     │←──FK────│
│   │ ├── checkpoint_id       │  logical FK   │ wtb_checkpoint_files    │         │
│   │ ├── checkpoint_data     │               │ wtb_batch_tests         │         │
│   │ └── metadata            │               │ wtb_batch_test_results  │         │
...
│   │                         │                                                    │
│   │ Marked only - not used  │               ┌─────────────────────────┐         │
│   │ LangGraph is primary    │               │ commits                 │◄────────│
│   │                         │               │ file_blobs              │  FK     │
│   └─────────────────────────┘               │ file_mementos           │         │
│                                             └─────────────────────────┘         │
│                                                                                  │
...
│   │ Concern                │ Storage                                        │   │
│   ├────────────────────────┼───────────────────────────────────────────────┤   │
│   │ Execution State        │ LangGraph Checkpointer (automatic)            │   │
│   │ Time Travel / Rollback │ LangGraph get_state_history()                 │   │
│   │ Thread Isolation       │ LangGraph thread_id                           │   │
│   │ WTB Domain Data        │ WTB SQLAlchemy UoW                            │   │
│   │ File Versions          │ FileTracker                                   │   │
│   └────────────────────────┴───────────────────────────────────────────────┘   │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
1. **LangGraph is PRIMARY** - Production-proven checkpointing (LangSmith uses this)
2. **WTB owns semantic layer** - Node boundaries, batch tests, evaluations are WTB concepts
3. **Thread-based isolation** - LangGraph `thread_id` for parallel batch tests
4. **Adapter handles translation** - `LangGraphStateAdapter` maps between WTB and LangGraph
5. **AgentGit is DEFERRED** - Interface marked only; implement later if needed
...
### 1. LangGraph Checkpointer Schema (PRIMARY)

LangGraph checkpointers automatically manage checkpoint tables. The schema varies slightly by backend:

```sql
-- ═══════════════════════════════════════════════════════════════════════════════
-- LANGGRAPH CHECKPOINTER SCHEMA (Managed by LangGraph - DO NOT MODIFY MANUALLY)
-- Source: langgraph-checkpoint-postgres / langgraph-checkpoint-sqlite
-- ═══════════════════════════════════════════════════════════════════════════════

-- Main checkpoint storage
CREATE TABLE checkpoints (
    thread_id TEXT NOT NULL,              -- Execution isolation key
    checkpoint_ns TEXT NOT NULL DEFAULT '',-- Namespace (for subgraphs)
    checkpoint_id TEXT NOT NULL,          -- Unique checkpoint identifier
    parent_checkpoint_id TEXT,            -- Chain reference for history
    type TEXT,                            -- Checkpoint type
    checkpoint BYTEA NOT NULL,            -- Serialized StateSnapshot
...
CREATE TABLE checkpoint_writes (
    ...
    task_id TEXT NOT NULL,                -- Pregel task identifier
    idx INTEGER NOT NULL,                  -- Write index
    channel TEXT NOT NULL,                -- State channel name
...
CREATE INDEX IF NOT EXISTS checkpoints_thread_id ON checkpoints(thread_id);
CREATE INDEX IF NOT EXISTS checkpoint_writes_thread_id ON checkpoint_writes(thread_id);
```

**Key Points:**
- Tables are **automatically created** by `checkpointer.setup()` (PostgreSQL) or on first use (SQLite)
- Schema is managed by LangGraph - do not modify manually
- `thread_id` is the primary isolation key for WTB executions
- `checkpoint_writes` stores pending writes for fault tolerance

### 2. AgentGit Database (DEFERRED - Reference Only)

> **⚠️ Status**: AgentGit adapter is **DEFERRED**. LangGraph is the primary implementation.
> This section is kept for reference only.

These tables exist in `agentgit.db` and are managed by the AgentGit package.
WTB does NOT use these when using LangGraphStateAdapter.
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- NODE BOUNDARIES (Updated 2026-01-15 - DDD Compliant)
-- ═══════════════════════════════════════════════════════════════════════════════
-- Semantic markers pointing to LangGraph checkpoints.
-- NOTE: Removed AgentGit-specific fields (internal_session_id, tool_count, etc.)
-- See docs/LangGraph/DDD_VIOLATION_REVIEW.md for rationale.

CREATE TABLE wtb_node_boundaries (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    
    -- WTB Execution Context (Domain Concept)
    execution_id TEXT NOT NULL,          -- WTB execution UUID = LangGraph thread_id
    
    -- Node Identification
    node_id TEXT NOT NULL,               -- Workflow node name
    
    -- Checkpoint Pointers (LangGraph checkpoint_id - TEXT, not INTEGER)
    entry_checkpoint_id TEXT,            -- CP where node appears in snapshot.next
    exit_checkpoint_id TEXT,             -- CP where node appears in snapshot.writes
    ...
    -- Metrics (node-level only, no tool_count)
    duration_ms INTEGER,
...
See [docs/LangGraph/WTB_INTEGRATION.md](../LangGraph/WTB_INTEGRATION.md) for full implementation.

```python
# ═══════════════════════════════════════════════════════════════════════════════
# wtb/infrastructure/adapters/langgraph_state_adapter.py
# PRIMARY: Anti-Corruption Layer using LangGraph Checkpointers
# ═══════════════════════════════════════════════════════════════════════════════
...
class LangGraphStateAdapter(IStateAdapter):
    ...
    def _create_checkpointer(self) -> BaseCheckpointSaver:
        if self._config.checkpointer_type == "memory":
            ...
            import sqlite3
            return SqliteSaver(sqlite3.connect(self._config.connection_string))
    ...
    def execute(self, initial_state: dict) -> dict:
        """Execute with automatic checkpointing at each super-step."""
    ...
    def get_checkpoint_history(self) -> List[dict]:
        """Get full history for time travel."""
        config = {"configurable": {"thread_id": self._current_thread_id}}
...
### AgentGitStateAdapter (DEFERRED - Reference Only)

> **⚠️ Status**: This implementation is **DEFERRED**. See LangGraphStateAdapter above.

```python
# ═══════════════════════════════════════════════════════════════════════════════
# wtb/infrastructure/adapters/agentgit_state_adapter.py (DEFERRED)
...
### Data Storage Distribution (Updated 2026-01-15)

| Component | Database | Owner | Purpose |
|-----------|----------|-------|---------|
| **checkpoints, checkpoint_writes** | **LangGraph Checkpointer** | **LangGraph** | **Execution state (PRIMARY)** |
...
| ~~agentgit tables~~ | ~~agentgit.db~~ | ~~AgentGit~~ | ~~DEFERRED - marked only~~ |

### State Adapter Implementations

| Implementation | Use Case | Storage | Status |
|----------------|----------|---------|--------|
| `LangGraphStateAdapter` | Production, Ray batch testing | PostgresSaver | 🆕 **PRIMARY** |
| `LangGraphStateAdapter` | Development | SqliteSaver | 🆕 **PRIMARY** |
| `LangGraphStateAdapter` | Unit tests | InMemorySaver | 🆕 **PRIMARY** |
| `InMemoryStateAdapter` | Unit tests (no LangGraph) | RAM | ✅ Available |
| `AgentGitStateAdapter` | Future | SQLite | ⏸️ **DEFERRED** |

### WTB UoW Implementations
...
1. **Production-proven** - LangSmith uses PostgresSaver in production
2. **Thread isolation** - Native `thread_id` for parallel batch tests
3. **Time travel** - Built-in `get_state_history()`, replay, fork
4. **Fault tolerance** - Pending writes preserved on node failure
5. **Serialization** - `JsonPlusSerializer` with encryption support
6. **Clean separation** - WTB owns its semantic concepts via adapters
7. **Repository + UoW** - Proper patterns for WTB domain data
8. **Ray-ready** - Works with Ray actors (each actor has own checkpointer)

### Design Principles

| Principle | Description |
|-----------|-------------|
| **LangGraph-First Persistence** | Use LangGraph checkpointers for state; SQLAlchemy UoW for WTB domain data |
| **Thread-Based Isolation** | LangGraph `thread_id` enables isolated checkpoint chains for parallel execution |
| **Single Checkpoint Type** | All checkpoints atomic at super-step level; node boundaries are pointers |
| **Interface Abstraction** | IStateAdapter, IUnitOfWork, IRepository enable swappable implementations |
| **Dependency Injection** | Factories enable test/production switching |
| **Data Lifecycle Aware** | Ephemeral → Ray ObjectRef; Persistent → PostgreSQL + LangGraph |
| **Index Strategy** | B-Tree for random access; BRIN for time-series; GIN for JSONB |

### LangGraph Reference

For more details, see:
- [LangGraph Reference](https://reference.langchain.com/python/langgraph/) - Core APIs
- [LangGraph Persistence](https://docs.langchain.com/oss/python/langgraph/persistence) - Checkpointers
- [docs/LangGraph/](../LangGraph/) - WTB integration documentation
